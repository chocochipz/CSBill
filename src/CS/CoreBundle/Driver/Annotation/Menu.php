...
namespace CS\CoreBundle\Driver\Annotation;

use CS\CoreBundle\Annotation;
...
/**
 * @DI\Service("cs_core.annotation_driver.menu")
 */
class Menu {
	...
	public function onKernelController(FilterControllerEvent $event)
	{
		...
		foreach (array_merge($this->reader->getClassAnnotations($object), $this->reader->getMethodAnnotations($method)) as $configuration) { //Start of annotations reading
			...
			if($configuration instanceof Annotation\Menu)
			{
				...
				$menu_builder = $controller[0]->get("cs_core.menu_builder");
				...
				$controller[0]->get("cs_core.menu.provider")->addItem($name, $menu);
			}
		}
	}
